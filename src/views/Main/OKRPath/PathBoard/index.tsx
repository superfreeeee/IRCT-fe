...
> = React.forwardRef(
  ({ source = DEFAULT_SOURCE, containerRef, listRef }, ref) => {
    // =============== state ===============
    ...
    const updateRelativeUsers = useCallback((targetNode?: PathNode) => {
      if (viewPointTypeRef.current === ViewPointType.Organization) {
        // 组织视图下不用更新
        return;
      }

      const { nodes, links } = sourceRef.current;
      const hasRelativeUsers =
        targetNode &&
        [EntityType.O, EntityType.Project].includes(targetNode.data.type);
    ...
    const createNode = (
      ...
      if (viewPointType === ViewPointType.Organization) {
        ...
        if (editModalTargetType !== EntityType.O) {
          // 只可能创建 O 节点
          console.error(
            `[PathBoard] create non O entity in Organization viewpoint`,
          );
          return;
        }
      } else {
    ...
    const editNode = (
      ...
      if (viewPointType === ViewPointType.Organization) {
        ...
        if (
          ...
          // remove old
          const removedLinks = linksRef.current.filter(
            ({ source, relative }) => {
    ...
    const deleteNode = (): number => {
      ...
      simulationRef.current
        .nodes(restNodesSelection.data())
        .alpha(0.2)
        .restart();

      // update inherit tree
      return originId as number;
    };
        ...
        switch (editModalResult.status) {
          case EditEntityModalResponseStatus.Confirm:
            if (editModalActionType === EditEntityModalActionType.Create) {
              entity = createNode(editModalResult.payload);
              outerClickNode(entity.id);
            } else if (editModalActionType === EditEntityModalActionType.Edit) {
              entity = editNode(editModalResult.payload);
              outerClickNode(entity.id);
            } else if (
              editModalActionType === EditEntityModalActionType.Delete
            ) {
              targetId = deleteNode();
              entity = { type: EntityType.Todo, id: '', originId: targetId };
            } else {
              console.warn(
            ...
            if (viewPointType === ViewPointType.Personal) {
              // update PathList in Personal viewpoint
              if (entity) {
                listRef.current.updateTree(
                  editModalActionType,
                  editModalSource,
                  entity,
                );
              } else {
                console.warn(`[PathBoard] edit but updateTree fail`);
              }
            }
          ...
          default:
            console.warn(
              `[PathBoard] unknown status ${editModalResult.status}`,
            );
    ...
    const _findNode = useCallback((nodeId: string) => {
      return nodesRef.current.data().find((node) => node.id === nodeId);
    }, []);

    const outerClickNode = useCallback((nodeId: string) => {
